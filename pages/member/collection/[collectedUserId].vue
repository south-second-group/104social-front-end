...
watchEffect(async () => {
  ...
  if (collectionDetails.value.collectedUsers?.userStatus?.commentCount !== 0)
        ...
        <div class="mb-4 grid w-full grid-cols-2 gap-6 gap-y-3">
          <div class="flex h-[35px] items-center">
            ...
            <span
              :class="{
                ...
                'italic text-neutral-400': collectionDetails.collectedUsers.nickNameDetails.nickName === '',
              }"
            >
              {{
                collectionDetails.collectedUsers.nickNameDetails.nickName || '對方保留'
              }}
            </span>
          </div>
          ...
          <div class="flex h-[35px] items-center">
            ...
            <span
              :class="{
                'font-montserrat': !useIsChineseFunc(
                  collectionDetails.collectedUsers.lineDetails.lineId,
                ),
                'italic text-neutral-400': collectionDetails.collectedUsers.lineDetails.lineId === '',
              }"
            >{{ collectionDetails.collectedUsers.lineDetails.lineId || '對方保留' }}</span>
          </div>
            ...
            <div
              ...
              :class="{
                'italic text-neutral-400':
                  collectionDetails.collectedUsers.introDetails.intro === '',
              }"
            >
              {{
                collectionDetails.collectedUsers.introDetails.intro || '對方保留'
              }}
            </div>
          </div>
          ...
          <div class="col-span-2 mt-3">
            ...
            <div class="mt-3 flex flex-wrap items-center justify-start gap-2 rounded-md">
              ...
              {{ collectionDetails.collectedUsers.tags.length === 0 ? '對方保留' : '' }}
            </div>
          </div>
        </div>
        ...
        <div v-if="isLoaded">
          ...
          <span class="hidden">
            {{
              createRenderValue(
                'industry',
                collectionDetails.matchListSelfSettingByUser.workInfo.industry,
              )
            }}
          </span>
        </div>
